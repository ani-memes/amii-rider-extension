...
class PluginMaster : ProjectManagerListener, Disposable, Logging {
  ...
  companion object {
    val instance: io.unthrottled.amii.rider.PluginMaster
      get() = ServiceManager.getService(io.unthrottled.amii.rider.PluginMaster::class.java)
  }

  private val projectListeners: ConcurrentMap<String, io.unthrottled.amii.rider.ProjectListeners> = ConcurrentHashMap()
  ...
  private fun registerListenersForProject(project: Project) {
    ...
    if (projectListeners.containsKey(projectId).not()) {
      ...
      projectListeners[projectId] =
        io.unthrottled.amii.rider.ProjectListeners(project)
